...
Use the bundled `scripts/setup_harness.py` for the active root instruction file's managed block. Run it first with `--dry-run`, adding `--purpose`, `--guardrail`, or `--done` only for stable project facts the user provided. Resolve the script path relative to this `SKILL.md`; do not assume the target project contains the plugin. The generator updates a non-empty root `AGENTS.override.md` when present because Codex gives it precedence over `AGENTS.md`.

Review the proposed block against the inspected project, then run the same command without `--dry-run`. If the script reports a marker conflict, stop and explain it; never repair ambiguous markers by deleting content. Review the resulting diff before continuing.
