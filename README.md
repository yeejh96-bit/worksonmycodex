...
Ask Codex: `Use $works-on-my-codex to set up WOMC in this project.` Add the product purpose, durable guardrails, or measurable completion criteria if they are known. Codex inspects the repository, previews and applies the managed block, reviews the diff, and runs the safe validation commands it records.
...
- Does not create Codex apps, MCP servers, hooks, status lines, or project permission configuration.
...
The last two absolute paths are development-machine examples. Use the corresponding `skill-creator` and `plugin-creator` validator paths in your Codex installation.
