...
조합은 [위임 정책 원본](skills/works-on-my-codex/assets/delegation-policy.md)에서 관리한다. 단순 탐색은 Luna Low, 범위가 좁은 웹 조사는 Luna Medium, 여러 출처 비교와 일반 구현은 Terra Medium을 기본으로 하며, 복잡한 분석과 중요한 검토에는 더 강한 조합을 선택한다. 필요한 프로젝트 스킬과 자료 경로도 전달하고, 메인이 반환된 근거·변경분을 검토해 통합한다. 위임의 전달·검토 비용도 있으므로 사용량 절감을 보장하지는 않는다.
