...
1.4.0부터 생성·갱신한 하네스에는 위임 판단 기준이 포함된다. 1.5.0부터는 이를 자동 분류 엔진이 아닌, 메인이 매 요청에서 한 번 내리는 내부 판단 계약으로 구조화한다. 사용자는 메인 모델과 추론 수준만 선택한다. 메인은 먼저 작업을 시작할 준비가 충분한지(`ready`), 일부만 안전하게 시작할 수 있는지(`partial`), 필요한 정보나 승인이 없어 멈춰야 하는지(`blocked`)를 가른다. 그 뒤 직접 처리할지(`direct`), 직접 처리와 위임을 함께 할지(`mixed`), 실행하지 않을지(`none`)를 별도로 정한다. 준비가 충분해도 작은 일은 직접 처리할 수 있다.

`partial`일 때는 막힌 부분과 결과·의존성이 분리된 안전한 로컬 작업만 진행한다. 위임할 만한 독립 조사·구현·검토가 있고 메인에게 병행할 일이 있을 때만 맡긴다. 조합은 [위임 정책 원본](skills/works-on-my-codex/assets/delegation-policy.md)에서 관리한다. 단순 탐색은 Luna Low, 범위가 좁은 웹 조사는 Luna Medium, 여러 출처 비교와 일반 구현은 Terra Medium을 기본으로 하며, 복잡한 분석과 중요한 검토에는 더 강한 조합을 선택한다.

위임 요청에는 담당 범위와 필요한 자료·스킬만 전달한다. 대화에만 있던 승인 조건, 허용 모델·예산·동시 인원, 네트워크·원격 푸시 제한도 빠짐없이 함께 전달한다. 계획을 세웠다는 것은 실제 호출 성공이 아니다. 메인은 호출·스킬 사용·반환 근거와 변경분을 확인해 통합하고 최종 검증과 보고를 책임진다.

`config.toml`이나 메인 모델 설정은 바꾸지 않으며, 위임 판단의 신뢰도 점수나 외부 저장 상태를 만들지 않는다. 세션 시작 훅도 사용하지 않는다. 지원되지 않는 모델·도구는 제한을 알리고 가능한 대안이나 메인 직접 처리로 정한다. 위임 금지, 권한 거부, 네트워크·도구 접근 실패는 모델을 높여 해결하려 하지 않는다. 기존 사용자 지정 모델·위임 제한을 우선한다. 적용·갱신한 프로젝트의 다음 세션부터 읽을 수 있으며, 기존 세션의 자동 재로딩은 별도 확인 대상이다.

선택 이유와 한계, 모의·실행 검증 방법은 [위임 정책과 검증](skills/works-on-my-codex/references/delegation.md)을 참고한다.
