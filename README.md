...
1.4.0부터 생성·갱신한 하네스에는 위임 판단 기준이 포함된다. 사용자는 메인 모델과 추론 수준만 선택한다. 메인은 분리 가능한 조사·구현·검토를 먼저 찾아 적극적으로 위임한다. 위임할 때마다 작업의 모호성·오류 영향·검증 난도에 맞는 모델과 추론 수준을 실제 호출 인수로 지정한다. 짧은 단일 작업이나 메인이 병행할 유용한 일이 없는 작업은 직접 처리한다. 긴 요청도 필요한 맥락을 독립적으로 나눌 수 있으면 위임 대상이다.

조합은 [위임 정책 원본](skills/works-on-my-codex/assets/delegation-policy.md)에서 관리한다. 메인은 위임할 때 현재 도구가 지원하는 가장 최신 범용 GPT 세대를 먼저 고르고, 그 세대 안에서 단순 탐색에는 빠른 모델, 일반 구현에는 균형형 모델, 복잡한 분석에는 강한 모델을 고른다. 이전 세대로 자동 대체하지 않으며, 새 모델이 나와도 정책의 모델명을 수정할 필요가 없다. 필요한 프로젝트 스킬과 자료 경로도 전달하고, 반환된 근거·변경분을 검토해 통합한다. 위임의 전달·검토 비용도 있으므로 사용량 절감을 보장하지는 않는다.

이 기능은 메인이 따르는 판단 지침이며 별도의 자동 분류 프로그램이 아니다. `config.toml`이나 메인 모델 설정은 바꾸지 않는다. 현재 환경의 위임 도구·모델·추론 지정이 지원돼야 하며, 미지원이면 적합한 대안 또는 메인 직접 수행으로 처리하고 제한을 알린다. 기존 사용자 지정 모델·위임 제한을 우선한다. 적용·갱신한 프로젝트의 다음 세션부터 읽을 수 있으며, 기존 세션의 자동 재로딩은 별도 확인 대상이다.
