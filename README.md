...
- 로컬 스킬을 만들면 `SKILL.md`의 `description`을 사용 조건으로 삼아 해당 스킬의 진입 경로를 하네스에 기록한다. 스킬 전용 참고 문서는 `SKILL.md`에서 계속 라우팅한다.
- 시작 문서·로컬 스킬·워크스페이스·검증 명령을 바꾸는 작업은 같은 작업이 끝나기 전에 하네스도 갱신한다.
